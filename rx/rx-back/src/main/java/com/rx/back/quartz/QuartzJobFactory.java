	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			...
			if (job == null) {
				...
				if (JobStatusVariable.LOCK) {
					logger.info("药品信息正在同步，请等待。。。");
				} else {
					logger.info("任务成功运行　任务分组和名称：" + job.getJobGroup() + "【" + job.getJobName() + "】 任务执行时间："
							+ job.getCronExpressionName());
					// 根据name 与 group组成的唯一标识来判别该执行何种操作……
					if (job.getJobGroup().equalsIgnoreCase(QuartzJobStaticConstants.QUARTZ_JOB_GROUP_NAME) && job
							.getJobName().equalsIgnoreCase(QuartzJobStaticConstants.QUARTZ_JOB_NAME_EVERY_MINUTE)) {
						// 每分钟执行
						this.everyMinuteTask();
					} else if (job.getJobGroup().equalsIgnoreCase(QuartzJobStaticConstants.QUARTZ_JOB_GROUP_NAME)
							&& job.getJobName()
									.equalsIgnoreCase(QuartzJobStaticConstants.QUARTZ_JOB_NAME_EVERY_DAY_TIME_ZERO)) {
						// 每天（00:00点）执行
						this.everyDayTask();
					} else {
		...
		try {
			...
			if (result_code.equals(RequestResultUtil.RESULT_CODE_SUCCESS)) {
				...
				try {// 解析JSON并保存内容
					if (StringUtils.isNotBlank(drugInfoJSON)) {
						...
						if (drugList != null && drugList.size() > 0) {
							...
							if (rows > 0) {
								...
								LogSyncDrug log = JSON.parseObject(logJSON, LogSyncDrug.class);
								log.setErrormsg(SyncDrugLogStatus.FAIL_SAVE_TO_DB.getValue());
								log.setStatus(SyncDrugLogStatus.FAIL_SAVE_TO_DB.getIndex());
								logSyncDrugService.insertSelective(log);
							}
						} else {
							// TODO 解析JSON为空
							...
							LogSyncDrug log = JSON.parseObject(logJSON, LogSyncDrug.class);
							log.setErrormsg(SyncDrugLogStatus.FAIL_LIST_NULL.getValue());
							log.setStatus(SyncDrugLogStatus.FAIL_LIST_NULL.getIndex());
							logSyncDrugService.insertSelective(log);
						}
					} else {
						// TODO 文件内容为空
						logger.error("药品JSON数据为空");
						LogSyncDrug log = JSON.parseObject(logJSON, LogSyncDrug.class);
						log.setErrormsg(SyncDrugLogStatus.FAIL_JSON_NULL.getValue());
						log.setStatus(SyncDrugLogStatus.FAIL_JSON_NULL.getIndex());
						logSyncDrugService.insertSelective(log);
					}
				} catch (Exception e) {
					e.printStackTrace();
					...
					LogSyncDrug log = JSON.parseObject(logJSON, LogSyncDrug.class);
					log.setErrormsg(SyncDrugLogStatus.FAIL_PARSE_OR_SAVE.getValue());
					log.setStatus(SyncDrugLogStatus.FAIL_PARSE_OR_SAVE.getIndex());
					logSyncDrugService.insertSelective(log);
				}
