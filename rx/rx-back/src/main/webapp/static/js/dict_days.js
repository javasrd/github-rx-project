...
function selectDetails(id){
	resetFun();
	var url = "back/dict-days/selectUpdateById";
	var params = {"id":id};
	$.post(url, params, function(res){
		...
		if(res!=null){
			...
			if(resp.result_code=="success"){
				$("#edit-dict-days-li").removeClass("hide");
				var days =resp.dictDays;
...
function remoteValid(){
	var valid = false;
	var url = "back/dict-days/checkValid";
	var id = $("#days-id").val();
	var code = $("#days-code").val();
	var name = $("#days-name").val();
...
function saveFun(){
	...
	if(id==null || id==""){
		...
		url = "back/dict-days/insert";
	}else{
		url = "back/dict-days/updateById";
	}
...
function deleteInfoAjaxRequest(id){
	var url = "back/dict-days/deleteById";
	var params = {"id":id};
	//util.loading();
	$.post(url, params, function(res){
		console.log(res);
...
function reloadInfoFun(){
	//操作成功后重新加载
	var href = "back/dict-days/selectItems?pagehelperFun=clickPageBtnRequestFun";
...
function clickPageBtnRequestFun(params){
	var action = "back/dict-days/selectItems";
...
function checkOne(){
    ...
    var flag = true;
    $("#dict-days-table tbody input[type='checkbox']").each(function(){
    	if(!$(this).prop("checked")){
    		flag = false;
    	}
    });
    if(flag){
    	$("#dict-days-table thead input[type='checkbox']").prop('checked', true);
    }else{
    	$("#dict-days-table thead input[type='checkbox']").prop('checked', false);
    }
}
/*
...
function checkAll(obj){
	$("#dict-days-table tbody input[type='checkbox']").prop('checked', $(obj).prop('checked'));
}

/**
 * 点击添加按钮显示添加编辑选项卡
 */
$("#add-dict-days-btn").click(function(){
	$("#edit-dict-days-li").removeClass("hide");
	$('#tabs-243687 a[href="#tab-2"]').tab('show');
});
...
function resetFun(){
	...
	$(":input","#save-form")  
	 ...
	 .removeAttr("checked")  
	 .removeAttr("selected");
	$("#edit-dict-days-li").addClass("hide");
}
