...
function selectDetails(id){
	resetFun();
	var url = "back/dict-mode/selectUpdateById";
	var params = {"id":id};
	$.post(url, params, function(res){
		...
		if(res!=null){
			...
			if(resp.result_code=="success"){
				$("#edit-dict-mode-li").removeClass("hide");
				var mode =resp.dictMode;
				$("#mode-id").val(mode.id);//ID
				$("#mode-code").val(mode.code);//编码
				$("#mode-name").val(mode.name);//名称
...
function remoteValid(){
	var valid = false;
	var url = "back/dict-mode/checkValid";
	var id = $("#mode-id").val();
	var code = $("#mode-code").val();
	var name = $("#mode-name").val();
...
function saveFun(){
	...
	if(id==null || id==""){
		...
		url = "back/dict-mode/insert";
	}else{
		url = "back/dict-mode/updateById";
	}
...
function deleteInfoAjaxRequest(id){
	var url = "back/dict-mode/deleteById";
	var params = {"id":id};
	//util.loading();
	$.post(url, params, function(res){
		console.log(res);
...
function reloadInfoFun(){
	//操作成功后重新加载
	var href = "back/dict-mode/selectItems?pagehelperFun=clickPageBtnRequestFun";
...
function clickPageBtnRequestFun(params){
	var action = "back/dict-mode/selectItems";
...
function checkOne(){
    ...
    var flag = true;
    $("#dict-mode-table tbody input[type='checkbox']").each(function(){
    	if(!$(this).prop("checked")){
    		flag = false;
    	}
    });
    if(flag){
    	$("#dict-mode-table thead input[type='checkbox']").prop('checked', true);
    }else{
    	$("#dict-mode-table thead input[type='checkbox']").prop('checked', false);
    }
}
/*
...
function checkAll(obj){
	$("#dict-mode-table tbody input[type='checkbox']").prop('checked', $(obj).prop('checked'));
}

/**
 * 点击添加按钮显示添加编辑选项卡
 */
$("#add-dict-mode-btn").click(function(){
	$("#edit-dict-mode-li").removeClass("hide");
	$('#tabs-243687 a[href="#tab-2"]').tab('show');
});
...
function resetFun(){
	...
	$(":input","#save-form")  
	 ...
	 .removeAttr("checked")  
	 .removeAttr("selected");
	$("#edit-dict-mode-li").addClass("hide");
}
