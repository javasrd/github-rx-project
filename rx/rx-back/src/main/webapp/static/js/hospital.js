...
function selectDetails(id){
	resetFun();
	var url = "back/hospital/select-by-id";
	var params = {"id":id};
	$.post(url, params, function(res){
		...
		if(res!=null){
			...
			if(resp.result_code=="success"){
				$("#edit-hospital-li").removeClass("hide");
				var hospital =resp.hospital;
				$("#hospital-id").val(hospital.id);//ID
				$("#hospital-code").val(hospital.code);//编码
				$("#hospital-name").val(hospital.name);//名称
				$("#hospital-comment").val(hospital.comment);//备注
...
function remoteValid(){
	var valid = false;
	var url = "back/hospital/checkValid";
...
function saveFun(){
	...
	if(id==null || id==""){
		...
		url = "back/hospital/insert";
	}else{
		url = "back/hospital/update-by-id";
	}
...
function deleteInfoAjaxRequest(id){
	var url = "back/hospital/logic-del-by-id";
	var params = {"id":id};
	//util.loading();
	$.post(url, params, function(res){
		console.log(res);
...
function reloadInfoFun(){
	//操作成功后重新加载
	var href = "back/hospital/select-items?pagehelperFun=clickPageBtnRequestFun";
...
function clickPageBtnRequestFun(params){
	var action = "back/hospital/select-items";
...
function checkOne(){
    ...
    var flag = true;
    $("#hospital-table tbody input[type='checkbox']").each(function(){
    	if(!$(this).prop("checked")){
    		flag = false;
    	}
    });
    if(flag){
    	$("#hospital-table thead input[type='checkbox']").prop('checked', true);
    }else{
    	$("#hospital-table thead input[type='checkbox']").prop('checked', false);
    }
}
/*
...
function checkAll(obj){
	$("#hospital-table tbody input[type='checkbox']").prop('checked', $(obj).prop('checked'));
}
...
function resetFun(){
	...
	$(":input","#save-form")  
	 ...
	 .removeAttr("checked")  
	 .removeAttr("selected");
	$("#edit-hospital-li").addClass("hide");
}
