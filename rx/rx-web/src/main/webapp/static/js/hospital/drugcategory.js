 ...
 * 		
 * 	
 * @param drugIndex 使用医生所选择的药品自动填充字段.
 * @returns
 */
function fillFieldByDefault(drugIndex){
	...
	//尝试自动计算数量.
	var quantity=-1;
	if(dosageArr.length>=0 && drugTimesArr.length>0 && treatmentDaysArr.length>0 && minSpecVal>0){
		quantity=dosageArr[0]*drugTimesArr[0]*treatmentDaysArr[0]/minSpecVal;
		//alert(quantity);		
...
$(function(){
	 ...
	 * data-bind:保存页号及页大小 	数据格式:pageNum-pageSize 
	 * 		如果data-bind为空字符串，则不做动作 
	 * 		否则当用户点击某页时，则发送与筛选相同的请求
	 *-----------------------------------------------------*/
